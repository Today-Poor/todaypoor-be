...
public class RankingResult extends BaseEntity {
    ...
    public static RankingResult create(UUID dailyRankingEventId, UUID userId, Integer rankNo, Integer totalAmount) {
        if (dailyRankingEventId == null) throw new IllegalArgumentException("dailyRankingEventId는 필수입니다.");
        if (userId == null) throw new IllegalArgumentException("userId는 필수입니다.");
        if (rankNo == null || rankNo < 1) throw new IllegalArgumentException("올바른 순위가 아닙니다.");
        if (totalAmount == null || totalAmount < 0) throw new IllegalArgumentException("올바른 총 지출액이 아닙니다.");
